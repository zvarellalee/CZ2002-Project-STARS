	...
	public UserUI authenticate(String username, String password) {
		...
		if (password.equals(decryptedPassword)) {
			if (currUser.getAdminAccess() == true) {
				UserUI newUI = new StaffUI(); // if admin, return staff UI
				return newUI;
			} else {
				if (LoginManager.checkAccessPeriod((Student)currUser) == true) {
					UserUI newUI = new StudentUI(); // else, return the student UI
					return newUI;
				}
			}
		} 
